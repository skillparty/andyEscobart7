...
describe("receivables.remove (soft delete)", () => {
  test("oculta la cuenta por cobrar de la lista pero conserva la fila", async () => {
    const t = convexTest(schema, modules);
    const { userId, as } = await withUser(t);

    const id = await as.mutation(api.receivables.create, {
      debtorName: "María",
      amount: 8500,
    });

    await as.mutation(api.receivables.remove, { id });
